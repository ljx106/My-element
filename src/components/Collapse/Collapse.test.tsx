...
describe('Collapse.vue', () => {
  beforeAll(() => {  // vitest钩子函数
    ...
    contents = wrapper.findAll('.vk-collapse-item__content') // 可以检查到vk-collapse-item__content，但是vk-collapse-item__wrapper不行
  ...
  test('点击标题展开/关闭内容', async () => {
    ...
    await secondHeader.trigger('click') // 触发点击事件
...
// test.skip() 可以跳过该测试  当某用例测试失败，但是整个项目需继续上线，可使用
// test.only() 仅运行该项测试，提高测试运行速度